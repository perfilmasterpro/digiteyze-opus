...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Super Admins can view all zapzap events') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Users can view their own workspace zapzap events') THEN
...
CREATE INDEX IF NOT EXISTS idx_zapzap_webhook_events_workspace_id ON public.zapzap_webhook_events(workspace_id);
CREATE INDEX IF NOT EXISTS idx_zapzap_webhook_events_external_id ON public.zapzap_webhook_events(external_id);
CREATE INDEX IF NOT EXISTS idx_zapzap_webhook_events_created_at ON public.zapzap_webhook_events(created_at DESC);
