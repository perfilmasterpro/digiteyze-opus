...
export class WebhookService {
  static extractEvent(payload: ZapZapPayload): ZapZapExtracted {
    ...
    const messageId = firstString(message.messageid, message.id, data.messageId, data.id);
    const chatId = firstString(message.chatid, chat.wa_chatid, chat.id, data.chatId);
    const externalId = firstString(payload.event_id, message.id, message.messageid, data.id, payload.delivery_id);
    ...
    const contactPhone = hasNewFormat
      ? cleanJid(firstString(message.sender_pn, chat.phone, message.chatid, chat.wa_chatid))
  ...
  static async logWebhook(workspaceId: string, provider: string, payload: ZapZapPayload): Promise<{ data: any; error: any }> {
    try {
      ...
      const { data: workspace, error: wsError } = await supabaseAdmin
        ...
        .eq('id', workspaceId)
        .maybeSingle();
      if (wsError || !workspace) return { data: null, error: new Error('Workspace inválido ou não encontrado') };
      ...
      if (data) await this.identifyLeadSecurely(data as WebhookLog);
      return { data, error };
    } catch (err) {
  ...
  private static async matchWithDdiFallback(workspaceId: string, phone: string): Promise<LeadMatchResult> {
    const base = normalizePhone(phone);
    const candidates = new Set<string>([base]);
    if (base.startsWith('55') && base.length >= 12) candidates.add(base.slice(2));
    else if (base.length >= 10 && base.length <= 11) candidates.add(`55${base}`);
  ...
  private static async identifyLeadSecurely(event: WebhookLog) {
    if (event.lead_match_status === 'matched' && event.lead_id) return;

    try {
      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
      const extracted = this.extractEvent(event.payload as ZapZapPayload);
      const phoneToMatch = extracted.contactPhone || (extracted.isInbound ? event.sender_phone : event.receiver_phone);

      if (!phoneToMatch) {
        await supabaseAdmin.from('zapzap_webhook_events').update({ lead_match_status: 'unmatched' }).eq('id', event.id);
        return;
      }
      ...
      await supabaseAdmin.from('zapzap_webhook_events').update({
        lead_id: matchResult.leadId,
        lead_match_status: tableStatus
      }).eq('id', event.id);

      if (matchResult.status === 'matched' && matchResult.leadId) {
        await this.createInteractionFromEvent({ ...event, lead_id: matchResult.leadId, lead_match_status: tableStatus });
      }
    } catch (err) {
  ...
  private static async createInteractionFromEvent(event: WebhookLog) {
    ...
    try {
      ...
      await createLeadInteraction(event.workspace_id, event.lead_id, {
        tipo: 'whatsapp',
        descricao: content,
        payload: {
          tipo: 'whatsapp',
          ...
          metadata: { raw_event: event.event, pushName: extracted.contactName }
        }
      }, true);

      // Resposta do lead interrompe a cadência automática para que o Follow-up assuma.
