...
function rowToLead(row) {
    ...
    return {
        ...
        empresa_id: row.empresa_id ?? d.empresa_id,
...
/* ─────────────────── API pública ─────────────────── */
export async function listLeads(workspaceId) {
...
export async function createLead(workspaceId, input) {
    const payload = {
...
export async function updateLead(workspaceId, id, input) {
    const { data, error } = await supabase
        .from("leads")
        .update({
        empresa_id: input.empresa_id ?? null,
...
export async function updateLeadStatus(workspaceId, id, status) {
