import { LEAD_ORIGENS, UFS } from "../../types/leads.types";
...
function mapOrigem(v) {
    if (!v)
        return undefined;
    const k = v.trim().toLowerCase().replace(/\s+/g, "_");
    if (LEAD_ORIGENS.includes(k))
        return k;
    if (k.includes("google") || k.includes("maps"))
        return "google_maps";
    if (k.includes("indic"))
        return "indicacao";
    if (k.includes("anunc") || k.includes("ads"))
        return "anuncio";
    if (k.includes("event"))
        return "evento";
    if (k.includes("parc"))
        return "parceria";
    if (k.includes("insta") || k.includes("social") || k.includes("rede"))
        return "redes_sociais";
    if (k.includes("site") || k.includes("form"))
        return "site";
    if (k.includes("inbound"))
        return "inbound";
...
function mapUf(v) {
    ...
    return UFS.includes(u) ? u : undefined;
}
