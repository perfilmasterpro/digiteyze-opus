...
function isThirdPartyListing(url) {
 ...
 */
export function parseGoogleMapsAddress(address) {
...
export const thunderbitProfile = {
    ...
    mapRow(row, normalizedHeaders) {
        ...
        if (extras.endereco) {
            ...
            if (parsed.cidade && !data.cidade)
                data.cidade = parsed.cidade;
            if (parsed.estado && !data.estado)
        ...
        const notes = [];
        if (isSponsored)
            notes.push("Tipo: Google Maps Patrocinado");
        if (thirdPartyListing)
            notes.push(`Listagem externa: ${thirdPartyListing}`);
        if (extras.endereco)
            notes.push(`Endereço: ${extras.endereco}`);
        const parsedBairro = extras.endereco
            ? parseGoogleMapsAddress(extras.endereco).bairro
            : undefined;
        if (parsedBairro)
            notes.push(`Bairro: ${parsedBairro}`);
        if (extras.avaliacao)
            notes.push(`Avaliação Google: ${extras.avaliacao}`);
        if (extras.qtd_avaliacoes)
            notes.push(`Nº avaliações: ${extras.qtd_avaliacoes}`);
        if (extras.preco)
            notes.push(`Faixa de preço: ${extras.preco}`);
        if (extras.status)
            notes.push(`Status Google: ${extras.status}`);
