...
function requireConfig() {
  ...
  if (missing.length) {
    throw new Error(`Configuração ZapZap incompleta: ${missing.join(", ")}`);
  }
}

export type ZapZapSendResult = {
  messageId: string;
  status: string;
};
...
export async function sendZapZapText(input: {
  ...
  if (!result?.messageId || !result.status) {
    throw new Error("ZapZap retornou resposta de envio sem messageId/status.");
  }
